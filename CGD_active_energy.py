...
class CgdTorch:
    def __init__(self,
        ...
        print(self.label_classes, "<- this is label_classes")
        print(self.loss, "<- this is loss")

    def patch_data(self):
        diff = self.train_imgs.size(1) - (self.train_imgs.size(1) // self.Pv) * self.Pv
        if diff == 0:
            print("diff is 0, skip patching the data")
        else:
    ...
    def confined_init(self, random_lambda=False):
        ...
        for i in range(self.malicious_count):
            ...
            model.attacked_non_member = self.test_imgs[attacked_non_member_idx], self.test_labels[
                attacked_non_member_idx]
        ...
        print(
            f"Experiment initialized, {self.Ph}x{self.Pv} honest participants, {self.malicious_count}x{self.Pv} malicious participants")
    ...
    def eq_train(self, threshold=1, attack_strength=1, selective_apply_grad=False):
        active_attack = False
        ...
        for epoch in range(self.num_iter):
            ...
            if epoch % self.stride == 0:
                ...
                print(
                    f'Epoch {epoch} - test acc {test_acc:6.4f}, test loss: {test_loss:6.4f}, mia_acc{mia_acc:6.4f}, tp={tp}, fn={fn}, fp={fp}, tn={tn}')
        ...
        recorder.to_csv(
            RECORDING_PATH + f"{self.dataset}_epoch_{self.num_iter}_Pv_{self.Pv}_Ph_{self.Ph}_nH_{self.n_H}_starts_"
...
class FedAvg(CgdTorch):
    def __init__(self, num_iter, train_imgs, train_labels, test_imgs, test_labels, Ph, Pv, n_H, init_lambda, batch,
                 ...
                 loss=torch.nn.CrossEntropyLoss(),
                 num_labels=0):
        super(FedAvg, self).__init__(num_iter, train_imgs, train_labels, test_imgs, test_labels, Ph, Pv, n_H,
                                     init_lambda, batch,
                                     dataset, verify_lrr=False, loss=loss, num_labels=num_labels)
        ...
        self.global_model = CGD_model_fc(Pv=Pv, n_H=n_H, in_length=self.train_imgs.size(1),
                                         out_length=self.label_classes)
